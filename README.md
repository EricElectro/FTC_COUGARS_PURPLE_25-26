# FTC Cougars Purple — 2025–26 Robot Repository

This repository contains the official **build instructions, wiring documentation, robot code, and game manual references** for the Cougars Purple FTC team (2025–26 season).

---

## 🔰 Start Here

| Task | Open This |
|------|-----------|
| Build the robot | [Build Instructions](Hardware/BOT_REV_DUO_FTC/Build_Instructions_REVDUO_V3-1/) |
| Wire the robot | [Wiring Documentation](Hardware/BOT_REV_DUO_FTC/DUO_Control_System/) |
| View or edit code | [Robot Code](Programming/Robot_Code/) |
| Read the game rules | [Game Manual](Season_2025_26/Game_Manuals/CHAPTERS/Ch11__Game_Rules.pdf) |
| Inspection checklist | [Inspection Checklists](Hardware/Checklists_Templates/) |

---

## 📁 Repository Structure


```
FTC_COUGARS_PURPLE_25-26/ <- You are here
├─ Hardware/
│  ├─ BOT_REV_DUO_FTC/
│  │  ├─ Build_Instructions_REVDUO_V3-1/
│  │  │  ├─ Hopper_Support/
│  │  │  ├─ Launcher/
│  │  │  └─ Superstructure/
│  │  └─ DUO_Control_System/
│  │     └─ image/
│  │        └─ README/
│  ├─ Checklists_Templates/
│  └─ Completed_Assemblies/
│     ├─ SN001/
│     └─ SN002/
├─ Programming/
│  └─ Robot_Code/
│     ├─ SN001/
│     └─ SN002/
└─ Season_2025_26/
   └─ Game_Manuals/
      └─ CHAPTERS/
```

## Directory

- [Hardware/](./Hardware/) - Contains all hardware-related resources, including build instructions for the BOT_REV_DUO_FTC robot, checklists, templates, and documentation of completed assemblies.
  - [BOT_REV_DUO_FTC/](./Hardware/BOT_REV_DUO_FTC/) - Contains build instructions and control system information for the BOT_REV_DUO_FTC robot.
  - [Checklists_Templates/](./Hardware/Checklists_Templates/) - Contains various checklists and templates for hardware assembly and maintenance.
  - [Completed_Assemblies/](./Hardware/Bot_Assemblies) - Contains documentation and details of completed robot assemblies, organized by serial number.

- [Programming/](./Programming/) - Contains all software-related resources, including robot code and programming documentation.
  - [Robot_Code/](./Programming/Robot_Code/) - Contains the robot code for different robot assemblies, organized by serial number.

- [Season_2025_26/](./Season_2025_26/) - Contains all resources related to the 2025-26 season, including game manuals and team documentation.
  - [Game_Manuals/](./Season_2025_26/Game_Manuals/) - Contains the game manuals for the 2025-26 season, organized by chapters.
